/* WA Private — app.js
   ...
   - Chat documents and real-time messages
   - Basic UI wiring: tabs, contacts, composer, settings
   - Placeholders: Agora calls + hidden feature uploads + Live Swap
*/
...
verifyOtpBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if (!doc.exists) {
      const appNumber = generateAppNumber();
      await db.collection('users').doc(res.user.uid).set({
        phone: res.user.phoneNumber, appNumber, createdAt: firebase.firestore.FieldValue.serverTimestamp()
      });
    }
  } catch (err) { alert(err.message) }
});
...
auth.onAuthStateChanged(async (user) => {
  if (user) { me = user; await openAppForUser(user); }
  else { authScreen.style.display='block'; appScreen.style.display='none'; }
});

/* ---------- utilities ---------- */
function generateAppNumber(){ return '+29'+Math.floor(100000000 + Math.random()*900000000); }
...
async function openAppForUser(user){
  ...
  myAppNumberEl.innerText = data.appNumber || data.phone || generateAppNumber();
...
addContactBtn.addEventListener('click', async ()=>{
  ...
  if (!appNum) return alert('Enter app number');
  ...
  let existsId = null;
  for(const d of candidate.docs){const parts=d.data().participants||[]; if(parts.includes(otherUid)&&parts.length===2){existsId=d.id;break;}}
  if(!existsId){ const chatRef=await db.collection('chats').add({participants:[me.uid,otherUid],createdAt:firebase.firestore.FieldValue.serverTimestamp(),lastMessage:''}); existsId=chatRef.id;}
...
async function openChat(chatId){
  ...
  messageListener=q.onSnapshot(snapshot=>{
    ...
    snapshot.forEach(doc=>{
      const m=doc.data();
      const cls=(m.from===me.uid?'me':'them');
      const el=document.createElement('div');
      el.className='message '+cls+(swapActive ? ' swap-'+cls : '');
...
/* ---------- send message + live swap ---------- */
sendMsgBtn.addEventListener('click', async ()=>{
  const txt=messageInput.value.trim();
  if(!txt) return; if(!currentChatId) return alert('Select a chat first');

  await db.collection('chats').doc(currentChatId).collection('messages').add({
    text:txt, from:me.uid, createdAt:firebase.firestore.FieldValue.serverTimestamp()
  });
  await db.collection('chats').doc(currentChatId).update({
    lastMessage:txt, lastUpdated:firebase.firestore.FieldValue.serverTimestamp()
  });

  if(swapActive){
    showSwapBanner();
    const swapped=txt.split('').reverse().join('');
    const chatDoc=await db.collection('chats').doc(currentChatId).get();
    const otherUid=chatDoc.data().participants.find(u=>u!==me.uid);
    await db.collection('chats').doc(currentChatId).collection('messages').add({
      text:swapped, from:otherUid, createdAt:firebase.firestore.FieldValue.serverTimestamp()
    });
  }
...
function showSwapBanner(){
  ...
  banner.innerText='Live Swap Active!';
...
hiddenVoiceFile.addEventListener('change', e=>{
  ...
  if(swapActive) audio.className='swap-preview';
  audio.play();
});
...
hiddenVideoFile.addEventListener('change', e=>{
  ...
  video.autoplay=true; video.width=200; video.height=150;
  if(swapActive) video.className='swap-preview';
...
unlockSwapBtn.addEventListener('click',()=>{
  ...
  alert('Live Swap '+(swapActive?'activated':'deactivated'));
});

/* ---------- calls placeholder (Agora) ---------- */
$('startCallBtn')?.addEventListener('click',()=>{
  const target=$('callWithNumber').value.trim();
  if(!target) return alert('Enter app number to call');
  alert('Starting call to '+target+' (Agora placeholder)');
  // Implement Agora RTC: join channel, publish local, subscribe remote
});
...
async function checkTrial(uid){
  ...
  if(Date.now()>data.trialEnd){ subscriptionModal.classList.remove('hidden'); appScreen.classList.add('hidden'); }
  else {$('trialText').innerText='Trial active: '+Math.ceil((data.trialEnd-Date.now())/(1000*60*60*24))+' days left';}
}
...
payButtons.forEach(b=>{
  b.addEventListener('click', async e=>{
    ...
    alert('Payment placeholder for plan '+plan);
